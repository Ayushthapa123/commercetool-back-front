...
describe("POST handler", () => {
  const orderId = "asdf-1234";
  const baseUrl = `http://localhost:4000/api/orders/${orderId}`;
  const context = {
  ...
  it("error updating order", async () => {
    ...
    const message = `Unable to update order with id: ${orderId}`;
    expect(json).toEqual({
      code: "Error",
      message: message,
      errors: [{}],
